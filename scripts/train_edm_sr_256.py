...
train_edm_sr_256.py
-------------------
Train a 64→256 super-resolution model using EDM (Karras et al., 2022)
...
Usage:
    # Single GPU
    python scripts/train_edm_sr_256.py \\
        --data_dir data/afhq/ \\
        --outdir checkpoints/edm_afhq_sr_256

    # Multi-GPU
    torchrun --standalone --nproc_per_node=4 scripts/train_edm_sr_256.py \\
        --data_dir data/afhq/ \\
        --outdir checkpoints/edm_afhq_sr_256
"""
...
def main():
    parser = argparse.ArgumentParser(description='Train EDM SR model (64→256)')

    # Directories
    parser.add_argument('--data_dir',     default='data/afhq/',
                        help='Path to AFHQ images')
    parser.add_argument('--outdir',       default='checkpoints/edm_afhq_sr_256_2',
    ...
    dist.print0()
    dist.print0('=' * 60)
    dist.print0('  EDM SR Training (64→256) with CDM Conditioning Aug')
    dist.print0('=' * 60)
